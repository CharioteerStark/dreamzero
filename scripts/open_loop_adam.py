#!/usr/bin/env python3
"""Offline open-loop evaluation for DreamZero on Adam data.

This script is a structural clone of scripts/open_loop_yam.py: it
reads one (observation, action) pair from the dataset per step, runs
inference, and compares the model's first-step prediction against the
ground-truth action at that index. The obs dict shapes match what
serve_wam.py builds at runtime, so a healthy result here means the
model + transform pipeline are fine and any "random" actions on the
real robot are coming from the wire/client layer.
...
# ---------------------------------------------------------------------------
# Adam layout  (from data/meta/modality.json)
# ---------------------------------------------------------------------------
...
EXPECTED_RESOLUTION = (640, 360)  # (W, H) — matches serve_wam.py default
...
class AdamDataset:
    """Reads LeRobot-style chunked parquet + per-camera MP4 for Adam."""
    ...
    def __init__(self, dataset_path: str):
        ...
        parquet_files = sorted(
            glob.glob(os.path.join(data_dir, "**", "episode_*.parquet"), recursive=True)
        )
        ...
        # Filename suffix uses each episode's own episode_index, not its
        # position in the file-listing.
        self.episode_id = [int(t.column("episode_index")[0].as_py()) for t in self.episodes]

        videos_root = os.path.join(dataset_path, "videos")
        self.video_dirs = {}
        for server_key, folder_name in VIDEO_CAMERAS.items():
            candidates = sorted(
                glob.glob(os.path.join(videos_root, "**", folder_name), recursive=True)
            )
            if candidates:
                self.video_dirs[server_key] = candidates[0]

        missing = set(VIDEO_CAMERAS) - set(self.video_dirs)
        if missing:
            raise FileNotFoundError(f"Missing camera dirs: {missing}")

        print(
            f"AdamDataset: {len(self.episodes)} episodes, "
    ...
    def get_state(self, idx) -> np.ndarray:
        ...
        return np.array(
            self.episodes[ep].column("observation.state")[row].as_py(),
            dtype=np.float64,
        )
    ...
    def get_action(self, idx) -> np.ndarray:
        ep, row = self._locate(idx)
        return np.array(
            self.episodes[ep].column("action")[row].as_py(),
            dtype=np.float64,
        )
    ...
    def get_frame(self, idx, server_key) -> np.ndarray:
        ...
        ep, row = self._locate(idx)
        ep_id = self.episode_id[ep]
        mp4 = os.path.join(self.video_dirs[server_key], f"episode_{ep_id:06d}.mp4")
...
def build_obs(dataset: AdamDataset, idx: int, prompt: str) -> dict:
    """Build an obs dict matching what GrootSimPolicy.forward() expects.

    Shapes match serve_wam.py's _convert_observation:
      video.*           : (1, H, W, 3) uint8
    ...
    for server_key in VIDEO_CAMERAS:
...
def get_gt_action_dict(dataset: AdamDataset, idx: int) -> dict:
    """Split the flat GT action vector at idx into per-key arrays."""
    action_flat = dataset.get_action(idx)
    return {k: action_flat[s:e] for k, (s, e) in ACTION_SLICES.items()}


# ---------------------------------------------------------------------------
# Plotting  (identical structure to open_loop_yam.py)
# ---------------------------------------------------------------------------
    ...
    for i in range(num):
        ...
        for k in ACTION_KEY_ORDER:
            if k in result.act:
                ...
                # First timestep of the chunk; squeeze may collapse (1, 24, 1) -> (24,)
        ...
        if i % args.log_every == 0:
            ...
            print(
                f"  [{i:>5d}/{num}] idx={idx} infer={elapsed:.3f}s prompt={prompt!r:.60}"
            )
...
def main():
    ...
    p.add_argument(
        "--model_path",
        default="./checkpoints/adam_stage_a_lora/checkpoint-5000",
        help="Path to model checkpoint dir (contains config.json, model.safetensors, experiment_cfg/).",
    )
    p.add_argument(
        "--dataset_path",
        default="./data",
        help="Root of Adam dataset (contains data/, videos/, meta/).",
    )
    p.add_argument("--device", default="cuda:0")
    p.add_argument(
        "--prompt",
        default="Pick up the yellow cube and place it on the pink circular pad.",
        help="Prompt to use when --use_dataset_prompt is not set.",
    )
    p.add_argument(
        "--use_dataset_prompt",
        action="store_true",
        help="Read annotation.task from the parquet instead of using --prompt.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Step between sampled indices (1 = consecutive frames, like open_loop_yam.py).",
    )
    p.add_argument("--output_dir", default="results_adam")
